...
<%
	...
	if(row > 0){
		...
		response.sendRedirect(request.getContextPath()+"/customer/checkId.jsp?msg="+msg);
		return;
	} else if(row == 0){
		msg = URLEncoder.encode("사용가능한 아이디입니다","utf-8");
		response.sendRedirect(request.getContextPath()+"/customer/checkId.jsp?useId="+id+"&msg="+msg);
		return;
	}
